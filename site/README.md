...
Araç danışmanının gerçek sohbet yanıtları için yalnızca sunucu ortamında `OPENAI_API_KEY` tanımlayın. Mevcut sohbet ekranı `POST /api/ai/chat` yolunu kullanır; eski `/api/ai/vehicle-advisor` yolu geçiş için aynı sunucu işlemini çağırır. Model `app/api/ai/chat/route.ts` içindeki tek `MODEL` sabitinde `gpt-5.4-mini` olarak ayarlanmıştır. Anahtarı istemci koduna veya depoya eklemeyin. Anahtar yoksa sohbet API'si 500 yanıtıyla yapılandırma eksikliğini bildirir.

Vercel yayını kullanılıyorsa anahtar proje ayarlarındaki Environment Variables bölümünde `Production` ortamına eklenmeli ve proje yeniden deploy edilmelidir. Anahtarın değeri kaynak dosyalara, Git geçmişine veya destek sohbetine yazılmamalıdır. Canlı doğrulamada `/api/ai/chat` uç noktasına gerçek bir soru sorulup başarılı yanıt alınmalıdır; yalnızca değişkenin varlığı yeterli doğrulama değildir.
