...
export function QuickValuationForm({ onVehicleChange }: { onVehicleChange?: (vehicle: VehicleContext) => void }) {
  ...
  const vehicle = { year: year ? Number(year) : undefined, brand, model, trim, mileage: mileage ? Number(mileage) : undefined };
  ...
  const changeBrand = (value: string) => { setBrand(value); setModel(""); setTrim(""); onVehicleChange?.({ ...vehicle, brand: value, model: "", trim: "" }); };
  const changeModel = (value: string) => { setModel(value); setTrim(""); onVehicleChange?.({ ...vehicle, model: value, trim: "" }); };
