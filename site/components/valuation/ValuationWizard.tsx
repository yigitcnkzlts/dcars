...
export function ValuationWizard({ vehicle, onClose }: Props) {
  ...
  const addPhotos = (files: FileList | null) => {
    ...
    setError("");
    setPhotos((current) => { const used = new Set(current.map((photo) => photo.area)); return [...current, ...incoming.map((file) => { const url = URL.createObjectURL(file); photoUrls.current.push(url); const suggested = areas.find((area) => area !== "Diğer" && !used.has(area)) ?? "Diğer"; used.add(suggested); return { file, url, area: suggested }; })]; });
  };
