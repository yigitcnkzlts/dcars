...
export function BrandSelect({ value, onChange, disabled = false }: Props) {
  ...
  return <fieldset className="brand-picker">
    ...
    {expanded ? <div className="brand-results" aria-label="Marka sonuçları">
      {matches.length ? matches.map((brand) => <button type="button" key={brand.name} onClick={() => select(brand.name)} className={value === brand.name ? "brand-result brand-result--active" : "brand-result"} disabled={disabled}><span className="brand-result__mark" aria-hidden="true">{brand.name.slice(0, 2).toLocaleUpperCase("tr-TR")}</span>{brand.name}</button>) : <button type="button" className="brand-result brand-result--custom" onClick={() => select(query.trim())} disabled={disabled}>“{query.trim()}” markasıyla devam et</button>}
    </div> : <><span className="brand-picker__hint">Sık seçilen markalar</span><div className="brand-popular">{popular.map((brand) => <button type="button" key={brand} onClick={() => select(brand)} aria-pressed={value === brand} disabled={disabled}>{brand}</button>)}</div></>}
