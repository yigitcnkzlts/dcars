...
type Props = { brand: string; value: string; onChange: (value: string) => void; grid?: boolean };

export function ModelSelect({ brand, value, onChange, grid = false }: Props) {
  ...
  if (grid) return <div className="model-picker valuation-field"><label htmlFor="vehicle-model-search">Modele göre ara</label><input id="vehicle-model-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Modele göre ara" autoComplete="off" /><small>{models.length} model seçeneği</small><h5>Tüm modeller</h5><div className="model-picker__grid">{matches.map((model) => <button type="button" key={model} aria-pressed={value === model} className={value === model ? "is-selected" : ""} onClick={() => onChange(model)}>{model}</button>)}</div>{!matches.length && <small>Model bulunamadı; manuel giriş kullanabilirsiniz.</small>}</div>;
