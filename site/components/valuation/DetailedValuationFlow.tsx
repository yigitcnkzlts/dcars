...
const steps = ["Araç Bilgileri", "Kilometre & Ekspertiz", "Ön Değerleme / Özet", "Kişisel Bilgiler / Randevu"];
...
type Draft = { selection: Partial<VehicleSelection>; mileage: string; color: string; accidentStatus: string; damageAmount: string; inspection: VehicleInspection; optionalEquipment: string[]; manualVehicle?: boolean };
...
export function DetailedValuationFlow({ onVehicleChange }: { onVehicleChange?: (vehicle: VehicleContext) => void }) {
  ...
  const [contact, setContact] = useState({ firstName: "", lastName: "", phone: "", email: "", city: "", plate: "", saleTiming: "", callRequested: false, preferredContactMethod: "Telefon" as ContactMethod, preferredContactTime: "" });
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        if (saved) {
          ...
          if (parsed.manualVehicle) { setManualVehicle(true); setVehiclePhase(4); }
        }
  ...
  useEffect(() => { if (draftRestored.current && !requestId) localStorage.setItem(draftKey, JSON.stringify({ ...draft, manualVehicle })); }, [draft, manualVehicle, requestId]);
  ...
  const setSelection = (field: keyof VehicleSelection, value: string | number) => {
    const order: (keyof VehicleSelection)[] = ["year", "brand", "model", "generation", "engine", "fuelType", "transmission", "version", "trim"];
    setDraft((current) => {
      const next = { ...current.selection, [field]: value };
      for (const key of order.slice(order.indexOf(field) + 1)) delete next[key];
      return { ...current, selection: next, color: field === "year" || field === "brand" || field === "model" ? "" : current.color };
    });
    setError("");
  };
  ...
  const startManualVehicle = () => {
    setManualVehicle(true);
    setVehiclePhase(4);
    setDraft((current) => ({ ...current, selection: { ...current.selection, generation: undefined, engine: undefined, fuelType: undefined, transmission: undefined, version: undefined, trim: undefined } }));
    setError("");
  };
  ...
  const next = () => {
    if (step === 0 && vehiclePhase < 4 && !manualVehicle) {
      if (vehiclePhase === 0 && (!selection.year || !selection.brand)) { setError("Model yılı ve marka seçin."); return; }
      if (vehiclePhase === 1 && !selection.model) { setError("Model seçin."); return; }
      if (vehiclePhase === 2 && !selection.fuelType) { setError("Yakıt tipini seçin veya aracınızı manuel girin."); return; }
      if (vehiclePhase === 3 && !selection.trim) { setError("Versiyon seçin veya versiyonumu bilmiyorum seçeneğini kullanın."); return; }
      setVehiclePhase((current) => current + 1); setError(""); return;
    }
    if (step === 0 && manualVehicle) {
      if (!selection.year || !selection.brand?.trim() || !selection.model?.trim() || !draft.color) { setError("Model yılı, marka, model ve rengi tamamlayın."); return; }
      setDraft((current) => ({ ...current, selection: { ...current.selection, engine: current.selection.engine?.trim() || "Bilmiyorum", fuelType: current.selection.fuelType?.trim() || "Bilmiyorum", transmission: current.selection.transmission?.trim() || "Bilmiyorum", version: current.selection.version?.trim() || "Bilmiyorum", trim: current.selection.trim?.trim() || "Bilmiyorum" } }));
      setError(""); setStep(1); return;
    }
    if (step === 0 && (!selection.year || !selection.brand?.trim() || !selection.model?.trim() || !selection.fuelType?.trim() || !selection.trim?.trim() || !draft.color)) { setError("Araç bilgilerini ve rengini tamamlayın."); return; }
    if (step === 0 && selection.trim === "Bilmiyorum") setDraft((current) => ({ ...current, selection: { ...current.selection, engine: "Bilmiyorum", transmission: "Bilmiyorum", version: "Bilmiyorum" } }));
  ...
  const submit = async () => {
    ...
    const payload: ValuationRequest = {
      year: selection.year, brand: selection.brand, model: selection.model, generation: selection.generation, catalogMatched: !manualVehicle && Boolean(variant), engine: selection.engine,
