...
import { VehicleSelectionSteps } from "./VehicleSelectionSteps";
...
export function DetailedValuationFlow({ onVehicleChange }: { onVehicleChange?: (vehicle: VehicleContext) => void }) {
  ...
  const chooseEngine = (chosen: NonNullable<typeof variant>) => {
    setDraft((current) => ({ ...current, selection: { ...current.selection, generation: chosen.generation, engine: chosen.engine, fuelType: chosen.fuelType, transmission: chosen.transmission, version: chosen.version, trim: undefined } }));
    setVehiclePhase(3);
    setError("");
  };
  ...
  const next = () => {
    if (step === 0 && vehiclePhase < 3 && !manualVehicle) {
      ...
      if (vehiclePhase === 2) { setError("Motor ve şanzıman kartlarından birini seçin veya aracınızı manuel girin."); return; }
      setVehiclePhase((current) => current + 1); setError(""); return;
    }
    if (step === 0 && manualVehicle) {
      if (!selection.year || !selection.brand?.trim() || !selection.model?.trim()) { setError("Model yılı, marka ve model bilgilerini tamamlayın."); return; }
      setDraft((current) => ({ ...current, selection: { ...current.selection, engine: current.selection.engine?.trim() || "Bilmiyorum", fuelType: current.selection.fuelType?.trim() || "Bilmiyorum", transmission: current.selection.transmission?.trim() || "Bilmiyorum", version: `${current.selection.engine?.trim() || "Bilmiyorum"} · ${current.selection.transmission?.trim() || "Bilmiyorum"}`, trim: current.selection.trim?.trim() || "Bilmiyorum" } }));
  ...
  return <section className="detailed-flow" id="teklif-formu" aria-labelledby="detailed-title"><div className="detailed-flow__intro"><span className="vehicle-eyebrow">D CARS · ÜCRETSİZ BAŞVURU</span><h2 id="detailed-title">Aracınızı birlikte tanıyalım.</h2><p>Bilgilerinizi tamamlayın, durumunu gösterin ve inceleme için gönderin. Fiyat, uzman değerlendirmesinden sonra iletilir.</p></div>
    ...
    <div className="detailed-panel">
      {step === 0 && <div className="detailed-stage"><StageTitle number="01" title="Aracınızı birkaç adımda seçin" text="Yıl ve markayla başlayın; ardından model, motor ve paketi seçin. Aracınız listede yoksa bilgileri kendiniz girebilirsiniz." /><VehicleSelectionSteps phase={vehiclePhase} manual={manualVehicle} selection={selection} onSelect={setSelection} onVariant={chooseEngine} onManual={startManualVehicle} onChangeVehicle={() => { setVehiclePhase(0); setManualVehicle(false); }} /></div>}
    ...
    </div>{error && <p className="detailed-error" role="alert">{error}</p>}<div className="detailed-actions"><button type="button" className="detailed-back" onClick={() => { if (step === 0) { setVehiclePhase((current) => Math.max(current - 1, 0)); setManualVehicle(false); } else setStep((current) => Math.max(current - 1, 0)); setError(""); }} disabled={step === 0 && vehiclePhase === 0}><ArrowLeft size={17} /> Geri</button><span>{step === 0 ? `${vehiclePhase + 1} / 4` : `${step + 1} / ${steps.length}`}</span>{step === steps.length - 1 ? <button type="button" className="button button--primary" onClick={submit} disabled={sending}>{sending ? "Gönderiliyor..." : "Teklif talebi gönder"} <ArrowRight size={17} /></button> : <button type="button" className="button button--primary" onClick={next}>{step === 0 && vehiclePhase === 3 ? "Araç durumuna geç" : "Devam et"} <ArrowRight size={17} /></button>}</div>
  </section>;
}
