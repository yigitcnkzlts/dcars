...
type Draft = { selection: Partial<VehicleSelection>; mileage: string; accidentStatus: string; damageAmount: string; inspection: VehicleInspection; optionalEquipment: string[] };
const initialDraft = (): Draft => ({ selection: {}, mileage: "", accidentStatus: "", damageAmount: "", inspection: initialInspection(), optionalEquipment: [] });
...
export function DetailedValuationFlow({ onVehicleChange }: { onVehicleChange?: (vehicle: VehicleContext) => void }) {
  ...
  const submit = async () => {
    ...
    const payload: ValuationRequest = {
      ...
      mileage: Number(draft.mileage), accidentStatus: draft.accidentStatus, damageAmount: draft.damageAmount,
...
function CatalogField({ label, field, value, options, disabled, onChange }: { label: string; field: SelectionField; value: string; options: string[]; disabled: boolean; onChange: (value: string) => void }) {
  ...
  const filtered = options.filter((option) => option.toLocaleLowerCase("tr-TR").includes(query.toLocaleLowerCase("tr-TR")));
  return <div className="valuation-field detailed-catalog-field"><label htmlFor={`catalog-${field}`}>{label}</label>{options.length ? <><input id={`catalog-${field}`} value={query || value} onChange={(event) => { setQuery(event.target.value); onChange(""); }} onFocus={() => setQuery("")} placeholder={`${label} ara`} disabled={disabled} autoComplete="off" /><div className="detailed-catalog-options">{filtered.map((option) => <button key={option} type="button" className={value === option ? "is-selected" : ""} disabled={disabled} onClick={() => { onChange(option); setQuery(""); }}>{option}</button>)}</div></> : <><input id={`catalog-${field}`} value={value} onChange={(event) => onChange(event.target.value)} disabled={disabled} placeholder={disabled ? "Önceki alanı seçin" : `${label} yazın`} /><small>{disabled ? "" : "Doğrulanmış katalog seçeneği yok. Bilmiyorsanız ‘Bilmiyorum’ yazın."}</small></>}</div>;
}
