...
export async function POST(request: Request) {
  try {
    ...
    if (!key) return Response.json({ error: "Yapay zekâ bağlantısı henüz kurulmadı. Site yöneticisinin AI_GATEWAY_API_KEY veya OPENAI_API_KEY eklemesi gerekiyor." }, { status: 503 });
    ...
    const response = await fetch(gatewayKey ? "https://ai-gateway.vercel.sh/v1/chat/completions" : "https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model: process.env.AI_GATEWAY_MODEL || (gatewayKey ? "openai/gpt-5.4-mini" : "gpt-5.4-mini"), messages: [{ role: "system", content: `${systemPrompt}\nFormda seçilen araç: ${context || "Seçilmedi"}. Bu alanı kullanıcı beyanı olarak değerlendir.` }, ...history, { role: "user", content: message }], ...(gatewayKey ? { max_tokens: 750 } : { max_completion_tokens: 750 }) }),
      signal: AbortSignal.timeout(20000),
    });
    if (!response.ok) { console.error("Vehicle advisor upstream status", response.status); return Response.json({ error: response.status === 401 || response.status === 403 ? "Yapay zekâ erişim anahtarı geçersiz veya yetkisiz." : "Yapay zekâ servisine ulaşılamıyor. Lütfen tekrar deneyin." }, { status: 502 }); }
